...
- This plugin will highlight instances where a text node is generated by calling `toString`, `toLocaleString`, `formatMessage` (from react-intl) and `t` (from react-i18next), though it is not able to identify text nodes returned from other functions. For example, the following code will not show an error.
