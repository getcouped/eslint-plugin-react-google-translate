  ...
  meta: {
    ...
    messages: {
      ...
      'text-node-preceeded-by-conditional':
        'Text nodes which are preceeded by a conditional expression, when rendered as a direct child of a JSX element, should be wrapped in an element (for example, a <span>) to prevent Google Translate causing a browser error while manipulating the DOM.',
    },
  },
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (
          ...
          conditionalSiblingsPreceedNode(node.parent)
        ) {
          context.report({
            node,
            messageId: 'text-node-preceeded-by-conditional',
