...
module.exports = {
  ...
  create(context) {
    ...
    function isStringifiableType(type) {
      if (!type) return false;
      if (type.isUnion()) {
        const parts = type.types.filter((part) => !isNullishType(part));
        return parts.length > 0 && parts.every(isStringifiableType);
      }
      if (type.isStringLiteral() || type.isNumberLiteral()) return true;
      const name = checker.typeToString(type);
    ...
    function returnsStringifiableValue(node) {
      try {
        ...
        return isStringifiableType(checker.getTypeAtLocation(tsNode));
      } catch {
        return false;
      }
    }
    ...
    function containsBareText(node) {
    ...
    return {
      ...
      Literal(node) {
        if (
          ...
          // `''` creates no text node: the reconciler's guard is `newChild !== ''`
          node.value !== '' &&
          !isWhitespace(node) &&
