...
module.exports = {
  ...
  create(context) {
    ...
    function isConditionalResult(node) {
      ...
      } else if (node.parent && node.parent.type === 'BinaryExpression') {
        return isConditionalResult(node.parent);
      }
      return false;
    }

    return {
      // conditionally rendered text nodes (string literals, or other literals that are coerced to strings)
      Literal(node) {
        if (
          !isWhitespace(node) &&
          node.value !== null &&
      ...
      // text nodes which are derived fom object properties
      MemberExpression(node) {
        if (
          node.property &&
          node.property.type === 'Identifier' &&
          isConditionallyRendered(node) &&
          isConditionalResult(node) &&
          isChildOfJSXEspressionContainer(node.parent) &&
          node.parent.parent &&
          isChildOfJSXElement(node.parent.parent) &&
