...
try {
  // wrap this in a try catch to support older node version which are not
  // compatible with @typescript-eslint/utils
  const utils = require('@typescript-eslint/utils');
  ESLintUtils = utils.ESLintUtils;
} catch (err) {
  console.log('Type checking unavailable', err);
}
  ...
  meta: {
    ...
    messages: {
      ...
      'text-node-preceded-by-conditional':
        'Text nodes which are preceded by a conditional expression, when rendered as a direct child of a JSX element, should be wrapped in an element (for example, a <span>) to prevent Google Translate causing a browser error while manipulating the DOM.',
    },
  },

  create(context) {
    // for non-ts projects, `parserServices` will be `null`
    ...
    try {
      ...
      console.log('No type information available');
    }
